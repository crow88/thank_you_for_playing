...
var Commonjs = Commonjs || (function() {
	...
	*/
	Popup.prototype.set = function(param) {
		...
		this._dom.on("click", function(e) { e.stopPropagation(); });
	...
	Popup.prototype.setClickInteraction = function() {
		...
		//画像ボタン用に拡縮を設定
		var on_css = {"transform":"scale(0.95,0.95)","-webkit-transform":"scale(0.95,0.95)"};
		var off_css = {"transform":"","-webkit-transform":""};
		var actions = {
	...
	return {
		...
		textSelect : function(param) {
			...
			$trigger.on('click', function(e){
				$target.select();
				$target[0].selectionStart=0; $target[0].selectionEnd=$target[0].value.length;
				e.preventDefault();
			});
		},
		...
		*/
		...
		replacePremiumCard : function(param) {
			...
			var manifest         = [
				{src: param.card_image, id:"replace_image_"+idol_base_id}
			];
		...
		createPopupByTemplate : function(p) {
			...
			var onTrigger = p.onTrigger || function() { popup.open() };
